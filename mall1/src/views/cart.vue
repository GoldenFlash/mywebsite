<template>
  <div>
              ...
              <li>
                <div class="cart-tab-1">
                  <div class="cart-item-check">
                    <a href="javascipt:;" class="checkbox-btn item-check-btn" :class="{'check':
                   item.checked}" @click="check(item)">
                  ...
                  <div class="cart-item-pic">
                    ...
                    <!-- v-bind="'static/'+ item.productImage" -->
                  </div>
                  ...
                  <div class="item-quantity">
                    <div class="select-self select-self-open">
                      <div class="select-self-area">
                        <a class="input-sub" @click="cartAdd(item,'substrat')">-</a>
                        <span class="select-ipt">{{item.productNum}}</span>
                        <a class="input-add" @click="cartAdd(item,'add')">+</a>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div class="cart-tab-5">
                  <div class="cart-item-opration">
                    <a href="javascript:;" class="item-edit-btn" @click="cartModalShow(item)">
          ...
          <div class="cart-foot-inner">
            <div class="cart-foot-l">
              <div class="item-all-check">
                <a href="javascipt:;" @click="toggleAll" >
                  <span class="checkbox-btn item-check-btn" :class="{'check':toggleAllFlag}">
            ...
            <div class="cart-foot-r">
              <div class="item-total">
                总额: <span class="total-price" >{{totalPrice | currency('￥')}}</span>
              </div>
              <div class="btn-wrap">
                <a class="btn btn--red"  :class="{'btn--dis':checkCount==0}" @click="checkedOut">
                  <!-- href="/#/address" -->
                 &nbsp;结&nbsp; 算&nbsp; </a>
              </div>
            </div>
    ...
    <modal v-bind:mdShow="cartModal" @close="cartModal=false">
      <p slot="message">删除此商品？</p>
      <div slot = "btnGroup">
          <a @click="deleteGood" class="btn btn--m">确定</a>
          <a @click="cartModal = false" class="btn btn--m">返回</a>
      </div>
    </modal>
    ...
    export default{
        ...
        components:{
         ...
         methods:{
          ...
          deleteGood(){
            var updateCartCount = this.productNum;
              axios.post("/users/cartDel",{"productId":this.productId}).then((res)=>{
                if(res.data.status=="0"){
                  this.cartModal = false;
                  this.$store.commit("updateCartCount",-updateCartCount)
                  this.init();
                }
            })
          },
          cartAdd(item,flag){
              axios.post("/users/cartAdd",{"productId":item.productId,'option':flag}).then((res)=>{
                if(res.data.status=="0"){
                  this.cartModal = false;
                  // this.init();
                  if(flag=='add'){
          ...
          check(item){
            axios.post("/users/checked",{"productId":item.productId,"checked":!item.checked}).then((res)=>{
              if(res.data.status=='0'){
          ...
          toggleAll(){
            ...
            axios.post("/users/toggleAll",{"checkAll":!this.toggleAllFlag}).then((res)=>{
              if(res.data.status == '0'){
                var Flag = !this.toggleAllFlag;
